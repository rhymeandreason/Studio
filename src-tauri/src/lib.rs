...
fn git_set_draft(app: AppHandle, repo: String, draft: String) {
    set_git_draft(&app, &repo, draft);
}

/// Plain helper behind `git_set_draft` (the `#[tauri::command]` can't be
/// `pub(crate)` without a macro-name clash). Lets `git::git_commit` clear the
/// saved draft after committing.
pub(crate) fn set_git_draft(app: &AppHandle, repo: &str, draft: String) {
    let mut list = read_git_windows(app);
