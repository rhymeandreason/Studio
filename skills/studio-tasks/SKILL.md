...
3. **In-person:** compute travel time to `location`, set
   `notify.leadMinutes = ETA + buffer` and a clear `notify.reason`, and set
   `inPerson: true`. (A native transit/ETA helper is planned; until then, reason
   about it or ask the user for their travel time, and state your assumption.)
