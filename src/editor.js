// Shared image-editor controller: the non-destructive, sidecar-backed edit
// surface (geometry/crop/tonal) plus the Export, Remove-background, and Extend
// modals. Extracted from media.js so both the media panel's lightbox *and* the
// standalone Image Editor tool (src/tools/image-editor.html) drive the same
// editor from a single source of truth.
//
// The media panel and a tool window are *separate documents*, so this module
// keeps using `document.getElementById` — each document just needs the editor +
// modal markup with matching ids. Anything grid- or project-specific (live
// thumbnails, grid reloads) is injected via `setEditorHooks` so the tool can
// pass no-ops.
...
// Overridden by the embedding context (media panel injects grid behaviour; the
// tool leaves them as no-ops).
let hooks = {
  // Live preview of the current edit onto the grid tile (media panel only).
  livePreview(_oriented, _editState) {},
  // Optimistic grid-tile thumbnail after a paste, before the re-bake lands.
  optimisticThumb(_path, _src, _editState) {},
  // Something was written to disk — refresh whatever lists the host shows.
  afterWrite() {},
  // Drop a cached thumbnail for a path that changed on disk.
  invalidateThumb(_path) {},
};
...
let editItem = null; // the item being edited: { path, name, ext }
// Three tiers of the *original* pixels (edits applied on top via the shader);
// the editor renders from the smallest loaded tier, upgrading as larger arrive.
let editThumb = null; // QuickLook thumbnail; loaded instantly on select
let editPreview = null; // ~2048px; loaded when the lightbox/editor opens
let editImg = null; // full resolution; loaded for export / remove-bg / extend
let editState = null; // current adjustments
let editSaveTimer = null;
let editDirty = false;

const THUMB_MAX = 768; // longest side (px) for the inline thumbnail
...
// --- Accessors used by the host (media panel) ------------------------------

export function getEditItem() {
  return editItem;
}
export function getCopiedEdits() {
  return copiedEdits;
}
export { ADJ_FIELDS };
...
// falling back to whatever is loaded so far.
...
// Lazily load the ~2048px preview (and the full-res image it's derived from).
// The full-res image doubles as the export / remove-bg / extend source.
export async function ensureFullRes() {
  if (editImg) return;
  const item = editItem;
...
// Load an item's edit controls + a fast thumbnail-resolution preview. Full
// resolution is deferred (see ensureFullRes) so selecting feels instant.
export async function loadEditor(item) {
  editItem = item;
  editThumb = null;
  editPreview = null;
  editImg = null;
  ...
  document.getElementById("lb-replace").hidden = !["png", "jpg", "jpeg"].includes(
    ext,
  );
...
// copy; export passes the full-res image. Cache keyed by source + geometry.
...
export function renderEditorPreview() {
  ...
  // Mirror the live edit onto the host's thumbnail, reusing the oriented canvas.
  hooks.livePreview(oriented, editState);
}
...
function positionCrop() {
  const canvas = document.getElementById("editor-canvas");
  const elc = document.getElementById("crop");
  const c = currentCrop();
  elc.style.left = `${c.x * canvas.width}px`;
  elc.style.top = `${c.y * canvas.height}px`;
  elc.style.width = `${c.w * canvas.width}px`;
  elc.style.height = `${c.h * canvas.height}px`;
}

let cropDrag = null;
let suppressLightboxClick = false;

// The host's lightbox backdrop-click handler calls this to swallow the
// synthetic click that follows a crop drag.
export function shouldSuppressLightboxClick() {
...
export function copyAdjustments() {
  if (!editState) return;
  const snap = {};
  ...
  const pasteBtn = document.getElementById("m-pasteadj");
...
export async function pasteAdjustments() {
  ...
  // Optimistic grid-tile update (host-specific), so the grid reflects the paste
  // with no blank gap.
  const src = previewSrc();
  if (src) hooks.optimisticThumb(editItem.path, src, editState);

  // Persist; the host's background re-bake swaps in seamlessly.
  editDirty = true;
  await flushEditSave();
  hooks.afterWrite();
...
export function setEditStatus(text) {
  const elc = document.getElementById("edit-status");
...
export async function bakeItemToBlob(item) {
...
// Open the Export dialog for the given context. Exported so the host can use it
// for batch (multi-select) exports too.
export function openWebExport(ctx) {
  webExportCtx = ctx;
  highlightFmt();
...
// --- Remove background (local segmentation) --------------------------------
...
// then blur the result so the margin reads as a soft continuation.
...
// Import the current image into Photos and open it in Edit.
...
// Wire the editor surface controls (rotate/flip/straighten/reset/crop) plus the
// Export / Remove-bg / Extend modals. Call once per document.
