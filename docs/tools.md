...
Tool windows load over `file://`, so browser save dialogs
(`showSaveFilePicker`) aren't available. Instead, tool windows are granted
the `core:default` Tauri capability (see
...
This writes into the **active Studio project's `designs/` folder** (see
`save_tool_export` in `src-tauri/lib.rs`) and returns the saved path. If no
project is active, it returns an error — `bento-grid.html`'s `saveFile()`
shows that as a status message.
